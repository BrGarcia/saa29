...
Favoritos usam PDFs reais de `docs/fim/` (versionado) via o mesmo helper de
...
def entrada(tmp_path: Path) -> Path:
    ...
    destino.mkdir()
    shutil.copy(Path("docs/fim") / NOME_PDF_ATA_36, destino / NOME_PDF_ATA_36)
    return destino
...
async def com_fim_map_ata(entrada: Path, tmp_path: Path, db: AsyncSession):
    ...
    pares = [
        p
        for p in catalog.carregar_fim_json(Path("docs/fim/fim.json"))
