...
async def criar_equipamento_helper(client: AsyncClient, headers: dict, dados: dict) -> dict:
    ...
    dados_copy["part_number"] = f"{dados['part_number']}-{uuid.uuid4().hex[:4].upper()}"
...
async def criar_tipo_controle_helper(client: AsyncClient, headers: dict, dados: dict) -> dict:
    dados_copy = dados.copy()
    dados_copy["nome"] = f"T{uuid.uuid4().hex[:4].upper()}"
...
async def criar_aeronave_helper(client: AsyncClient, headers: dict, dados: dict) -> dict:
    dados_copy = dados.copy()
    dados_copy["matricula"] = uuid.uuid4().hex[:4].upper()
...
class TestInstalacoes:
    ...
    async def test_instalar_item_em_aeronave(
        ...
        response = await client.post(f"{EQUIP_URL}/itens/{item['id']}/instalar", json={"aeronave_id": aeronave["id"], "data_instalacao": hoje}, headers=headers)
        assert response.status_code == 201
    ...
    async def test_remover_item_de_aeronave(
        ...
        hoje = date.today().isoformat()
        inst = (await client.post(f"{EQUIP_URL}/itens/{item['id']}/instalar", json={"aeronave_id": aeronave["id"], "data_instalacao": hoje}, headers=headers)).json()
