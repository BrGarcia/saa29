...
- **Correção proposta:** migrar `service.py` para `domain_exc.EntidadeNaoEncontradaError`/`ConflitoNegocioError`, eliminando o *string-matching* do router — mesma direção já recomendada para `panes` em `achados_panes.md` (MELHORIA-13).
...
- **Paginação em `GET /aeronaves/`**: já implementada (`skip`/`limit`), diferente de `auth.listar_usuarios` (achado 22 em `achados_auth.md`) — não é achado aqui.
