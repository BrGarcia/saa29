...
|---|---|---|
| **9 achados não corrigidos em `service.py`** (3 BUG, 3 RISCO, 3 MELHORIA + 1 dúvida) | Duas passadas de análise (automatizada + leitura manual linha a linha), ainda sem sessão de correção. Os três BUG: (1) rótulo de edição inválido pode derrubar `/api/busca` e `/api/status` com 500 em vez do erro limpo; (2) `buscar_no_catalogo` não escapa `_`/`%` do termo digitado — única função do arquivo que quebra o padrão SEC-07 já usado em `buscar_por_mensagem_fim`/`listar_fim_por_ata`, e `_` é onipresente nos nomes do acervo; (3) **`sincronizar_fim_map` apaga o mapa do FIM inteiro e o reaponta para a edição indexada por último, não para a VIGENTE** — acontece no fluxo normal de publicação (publicar edição nova antes de ativar), fazendo `GET /api/fim` e o bloco de procedimentos na pane apontarem para outra edição; a faixa "REVISÃO ANTERIOR" do viewer torna o erro visível, não silencioso. Mais 3 RISCO (corrida em favoritar, colisão de basename no mapa do FIM, `limit` sem clamp em 2 funções) e 3 melhorias. Ver [`dividas/achados_service.md`](dividas/achados_service.md) para correção proposta, risco de regressão e **ordem sugerida de correção**. | `app/modules/publicacoes/service.py` |
