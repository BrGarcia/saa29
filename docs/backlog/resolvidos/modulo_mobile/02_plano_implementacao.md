...
**Bug encontrado e corrigido em rodada posterior (2026-08-20):** `renderizarAnelProgresso` (`inspecoes_mobile.js`) só contava tarefas com `status === 'CONCLUIDA'` para o anel de progresso — tarefas marcadas `N/A` (não aplicável, uma das 3 opções da folha de ação da Etapa 4) ficavam de fora da contagem e o anel nunca chegava a 100% numa inspeção com pelo menos uma tarefa N/A, mesmo com todas as tarefas resolvidas. Corrigido para contar `CONCLUIDA` **ou** `N/A` como resolvida.
...
**Bug encontrado e corrigido em rodada posterior (2026-08-20):** o `hojeIso` da folha de execução vinha de `new Date().toISOString().slice(0, 10)`, que usa o dia em **UTC**. Em fusos negativos (Brasil, UTC-3), isso adianta a data em um dia durante a noite (ex.: 23h de 19/08 local vira "2026-08-20" em UTC) — exatamente o cenário que o próprio `max` nativo do input deveria impedir (não se pode registrar execução "no futuro"), então a UI ficava inconsistente com o relógio local do usuário justo no horário de maior risco de erro (troca de turno noturna). Corrigido com um helper `hojeLocalIso()` em `app.js` (compartilhado com o desktop) que monta a string a partir de `getFullYear()/getMonth()/getDate()` locais. Mesma correção aplicada às datas de remoção/instalação do Inventário (Etapa 6, ver abaixo) — os três pontos do mobile que enviam "hoje" para o backend usavam o mesmo padrão incorreto.
...
**Feito**, mas com um bug real no que foi dado como certo na primeira rodada: a nota original dizia que `InventarioItemOut` "já expunha `equipamento_id`" e por isso o ajuste de schema previsto seria dispensável — só que `equipamento_id`, em `_montar_linha_inventario` (`equipamentos/service.py`), é `slot.id` (o próprio slot, campo de "Compatibilidade Frontend V1"), **não** o id do modelo de equipamento. `inventario_mobile.js` usava esse valor como `equipamento_id` na query `GET /equipamentos/itens/?equipamento_id=...` para listar itens de estoque compatíveis — ou seja, a tela "Instalar" buscava itens pelo id do slot, nunca pelo id do modelo/PN real, o que devolvia lista vazia (ou, na pior hipótese, itens de um modelo errado caso algum id colidisse) em qualquer slot testado. Bug encontrado e corrigido em rodada posterior (2026-08-20): `modelo_id: uuid.UUID` adicionado de fato a `InventarioItemOut` (`equipamentos/schemas.py`) e populado em `_montar_linha_inventario` a partir de `slot.modelo_id`; `inventario_mobile.js` passou a usar `item.modelo_id` tanto na checagem de slot sem modelo vinculado (guarda agora efetivamente morta — `SlotInventario.modelo_id` é `nullable=False` no banco, todo slot sempre tem modelo; mantida por defesa em profundidade, sem custo) quanto na query de itens de estoque.

Mensagem de estoque vazio implementada literalmente como especificado. Nenhum teste novo automatizado nesta rodada para o fluxo remover/instalar em si (a lógica de negócio já é testada no nível de serviço/router de `equipamentos`, fora do escopo mobile) — mas a suíte completa de `equipamentos`/`inventario` (107 testes) foi reexecutada após o fix de `modelo_id` e continua verde, confirmando que a mudança de schema não quebrou nenhum consumidor existente. A verificação ponta a ponta desta etapa em navegador real continua pendente — mesma situação de "não testado em aparelho físico" já registrada na Etapa 1 para a câmera, e sem browser automation disponível neste ambiente para supri-la.
...
## 8. Etapa 7 — PUBLICAÇÕES + acabamento PWA ✅ CONCLUÍDA (com ressalva de verificação física, ver nota)

**Feito nesta rodada (2026-08-20), fechando a pendência registrada acima:**

- **§8.1 — Normalização de `mobile/publicacoes.html`:** o template passou a usar `.mobile-card`, `.mobile-field`/`.mobile-input`/`.mobile-select` e `.btn-mobile-action` (`.btn-mobile-accent`/`.btn-mobile-ghost`) — todas classes que já existiam em `mobile.css` (criadas nas Etapas 3-6 para outras telas), não foi preciso criar nada novo. Isso eliminou o `style="color: var(--text-primary)"` inline que compensava o `.card` branco do desktop herdado sobre a casca escura: `.mobile-card` não define cor de texto própria, herda `--mobile-text` do `body.mobile-body`, então o texto já nasce legível sem workaround. O `<div id="pub-status-vazio">` manteve `style="display:none"` inline (em vez de `hidden`) de propósito — a regra global `[hidden] { display: none !important; }` (Etapa 3) venceria o `statusVazio.style.display = "block"` que `publicacoes.js` usa para reexibi-lo.
  - `publicacoes.js` (compartilhado com o desktop) ganhou consciência de contexto: `const isMobile = document.body.classList.contains("mobile-body")` escolhe `.mobile-card`/`.card` e as variáveis de cor certas (`--mobile-text-muted` vs. `--text-secondary`, `--mobile-card-border-soft` vs. `--border-color`) nos três pontos que renderizam HTML via JS (`renderResultado`, card de "Nenhum resultado", lista de resultados do FIM) — exatamente a abordagem que o plano original apontava como "conserto de verdade", sem duplicar a lógica de busca/renderização.
  - Teste de regressão: `test_mobile_publicacoes_usa_classes_mobile_nao_desktop` e `test_mobile_publicacoes_requer_autenticacao` em `tests/unit/test_mobile.py`.
- **§8.2 — Viewer de PDF em tela pequena:** adicionado um breakpoint `@media (max-width: 480px)` em `publicacoes.css` (a barra de ferramentas do viewer tem 5 grupos de controles que, empilhados por `flex-wrap` num viewport de 393px, comiam uma fração desproporcional da altura útil): esconde o subtítulo do cabeçalho, joga o campo de busca para uma linha própria em largura total (em vez de espremido ao lado dos outros grupos), e reduz padding da barra e do palco do canvas para devolver altura ao PDF em si.
  **Ressalva importante:** isto foi dimensionado por leitura de código (tamanho de botões/inputs, contagem de grupos que fazem wrap), não por captura visual real — este ambiente não tem Playwright/navegador automatizável disponível para screenshot em viewport emulado. Gestos de pinça/zoom nativos do navegador sobre o `<canvas>` não foram exercidos (dependem do navegador/SO, não de código deste projeto). Recomenda-se uma checagem visual num dispositivo ou emulador antes de dar isto por definitivamente resolvido.
- **§8.3** (já feito na rodada anterior): `manifest.json` com `id`/`scope`/`description`, paleta v2.
- **§8.4 — Fechamento:** `ruff check .` limpo nos arquivos tocados por este módulo (os 11 erros remanescentes no repositório são pré-existentes, em `publicacoes/router.py`, `publicacoes/service.py` e scripts fora do escopo mobile — não introduzidos por este trabalho); `pytest tests -q` → 742 passando (740 + 2 testes novos desta rodada, §8.1), as mesmas 3 falhas pré-existentes do acervo real de publicações ausente neste ambiente, nenhuma nova. `docs/ROADMAP.md` e `docs/methodology/NEXT.md` atualizados (PWA movida para concluído, com a ressalva de offline de gravação fora de escopo e de instalação em aparelho físico não verificada).
...
6. Marcar tarefa de inspeção como `CONCLUIDA`; inspeção muda para `EM_ANDAMENTO`, progresso sobe. ❌ não testado em navegador (Etapa 4 implementada e coberta por `pytest`, mas sem verificação visual manual — sem browser automation neste ambiente)
7. Registrar execução de vencimento; status muda e aparece no histórico. ❌ não testado em navegador (Etapa 5 implementada e coberta por `pytest`, mesma limitação acima)
8. Remover e depois instalar um item no mesmo slot; conferir reflexo em `/inventario` no desktop. ❌ não testado em navegador (Etapa 6 implementada e coberta por `pytest`, mesma limitação acima)
...
- [x] Etapa 7 — Publicações normalizada (classes `mobile.css`, ver §8.1), PWA instalável por código (manifest + SW + ícones), docs atualizados (`ROADMAP.md`, `NEXT.md`, este documento). Não testado em dispositivo físico — ver ressalva abaixo.
- [x] `pytest tests -q` e `ruff check .` limpos em todo o módulo mobile (742 passando, incluindo os 2 testes novos de §8.1; as mesmas 3 falhas pré-existentes não relacionadas do acervo real de publicações ausente neste ambiente — nenhuma nova).
...
- [x] **Pendência da rodada anterior fechada por código/teste automatizado, não por navegador:** as telas de checklist de inspeção, vencimentos e inventário (Etapas 4-6) e a normalização de Publicações (Etapa 7) só foram exercidas via `pytest` + leitura de código nesta rodada — sem browser automation disponível neste ambiente (sem Playwright/MCP), não houve como repetir a verificação visual manual que as Etapas 1-3 tiveram. **Continua pendente, agora explicitamente, para quem tiver acesso a navegador/dispositivo:** confirmar visualmente as 4 telas do hub, a instalação do PWA em Android/iOS físico, a captura de foto pela câmera real, e a sessão de 15 min sem queda para `/login` — nenhum desses itens pode ser fechado só com testes automatizados de backend/HTML.
