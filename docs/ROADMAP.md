...
- [x] **PWA instalável (Progressive Web App)**: manifest (`id`/`scope`/`description`, ícones, `theme_color`), Service Worker no escopo certo (`/sw.js`, `Service-Worker-Allowed: /`) e retrofit visual v2 aplicado a todo o `/m/` (Frota agregada, Hub de 4 abas por aeronave, Panes, Inspeções, Vencimentos, Inventário e Publicações — ver `docs/backlog/modulo_mobile/02_plano_implementacao.md`). **Modo Offline (gravação sem rede) não incluído** — decisão explícita de escopo (spec §1.2): o SW cacheia assets estáticos para abrir o app sem rede, mas toda escrita (concluir pane, executar vencimento, etc.) ainda exige conectividade. Instalação em tela de início testada apenas via checklist de código (manifest/ícones/SW); não verificada em aparelho físico Android/iOS nesta rodada.
