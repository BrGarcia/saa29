...
- [x] **Segurança (Logout - Critical):** Fazer `clearAuth()` chamar `POST /auth/logout` no backend antes de limpar o storage no client; resolver dependência de blacklist em memória para ambientes multi-workers.
- [x] **Infraestrutura (DB - High):** Remover o rewrite silencioso da configuração de `DATABASE_URL` para SQLite em ambiente de desenvolvimento. O sistema deve validar e respeitar a URL informada.
- [x] **Confiabilidade (Uploads - High):** Interromper o fluxo de criação da pane quando ocorrer falha no envio do anexo, checando corretamente o status da resposta HTTP no frontend ou criando um fluxo transacional único.
- [x] **Segurança (Token - Medium):** Migrar a autenticação do frontend que hoje usa `localStorage` explícito para Cookies com `HttpOnly` e `SameSite`, bloqueando o acesso direto às proteções de rotas via navegador.
...
- [x] **Manutenção (Code Cleanup - Low):** Excluir esquemas (`LoginRequest`), imports mortos (`IntegrityError`) e código/métodos que não estão conectados à API ou à interface.
