...
def has_privilege(current_user: Usuario) -> bool:
    """Verifica se o usuário tem papel privilegiado (ENCARREGADO ou ADMINISTRADOR)."""
...
def format_event_for_user(event: CalendarEvent, current_user: Usuario) -> schemas.CalendarEventPayload:
    ...
    # 10.2 — censura completa para eventos privados de terceiros
    if _should_censor(event, current_user):
        ...
        return schemas.CalendarEventPayload(
            ...
            icon="L",
            owner_trigram=None,        # não vazar identidade
            notes=None,
            source="calendario",
            event_type_id=None,        # não vazar tipo (deduz cor/natureza)
            owner_user_id=None,        # não vazar UUID do dono
            can_edit=False,
            can_delete=False,
        )
    ...
    stmt = (
        ...
        .where(
            and_(
                ...
                CalendarEvent.deleted_at.is_(None),   # 10.5 — excluir soft-deletes
            )
        )
        .options(
        ...
        .limit(_QUERY_LIMIT)   # 10.4 — proteção contra DoS
    )
    result = await db.execute(stmt)
    events = list(result.scalars().all())

    if len(events) == _QUERY_LIMIT:  # 10.4 — alerta quando limite é atingido
        logger.warning(
...
async def _get_inspection_events(
    ...
    stmt = (
        ...
        .where(
            ...
            # 10.1 — filtrar apenas inspeções em curso
