...
async def listar_tipos_evento(
    ...
    tipos = await service.list_event_types(db, only_active=only_active)
    return [schemas.EventTypeOut.model_validate(tipo) for tipo in tipos]
...
async def criar_tipo_evento(
    ...
    # LookupError/PermissionError já viravam domain_exc no service (MELHORIA-05)
    # — HTTPException tipada se propaga sozinha, sem precisar de except aqui.
    try:
        tipo = await service.create_event_type(db, data)
    except ValueError as exc:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(exc)) from exc
    return schemas.EventTypeOut.model_validate(tipo)
...
async def atualizar_tipo_evento(
    ...
    try:
        tipo = await service.update_event_type(db, type_id, data)
    except ValueError as exc:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(exc)) from exc
    return schemas.EventTypeOut.model_validate(tipo)
